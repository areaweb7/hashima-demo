...
def post_to_wordpress(config, article_content_html, seo_settings, category_id, featured_media_id=None):
    wp_config = config['wordpress']
    ...
    post_data = {
        ...
        'status': "publish", 'categories': [category_id]
    }
...
if __name__ == "__main__":
    ...
    seo_settings = {
        ...
        "slug": f"welcome-vietnam-technical-interns-{datetime.now().strftime('%Y%m%d')}",
    ...
    post_to_wordpress(config, final_html, seo_settings, CATEGORY_ID, feat_id)
    print("--- 投稿プロセスが完了 ---")
